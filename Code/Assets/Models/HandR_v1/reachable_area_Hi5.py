...
def computeReachArea(vertices, touchT, touchI, touchM, touchR, touchP, nbgraph, labelAbs, labelOrd):
    ...
    try:
        ...
        print(touchR)
        print(touchP)
        # Get the convex hull 凸面 外壳 convexhull（凸包算法）：一组平面上的点，求一个包含所有点的最小凸多边形，这个最小凸多边形就是凸包。
        ...
        # hull_object.volume 凸包的体积
        # hull_object.area 凸包的面积
        ...
        reachableP = hull_p.area * cm
        percP = (reachableP / objectarea) * 100
        print('Pinky Reachable Area: ' + str(round(reachableP, 3)) + ' (' + str(round(percP, 3)) + '%) ')
        ...
        plt.plot(touchP[:, 0], touchP[:, 1], '.', color='#ff9557', label='Pinky')

        # Plot the line between the hull points of touched area
        # simplices: 形成凸包的简单面的点的索引
        ...
        for simplex in hull_p.simplices:
            plt.plot(touchP[simplex, 0], touchP[simplex, 1], '-', color='#ff9557')
        ...
        # Get the centroid(形心) of thumb reachable area
        distance = []
        ...
        pinkytext = str('Pinky Reachable Area: ' + str(round(reachableP, 3)) + ' (' + str(round(percP, 3)) + '%) ')

        # 设置X,Y轴的标签
        plt.xlabel(labelAbs)
        plt.ylabel(labelOrd)
        return (objecttext, thumbtext, indextext, middletext, ringtext, pinkytext)
    # return(objecttext)
...
def ConvexHull3D(vertices, touchT, touchI, touchM, touchR, touchP):
    ...
    try:
        ...
        touchPx = np.array([np.array([x[0]]) for x in touchP])
        touchPy = np.array([np.array([x[1]]) for x in touchP])
        touchPz = np.array([np.array([x[2]]) for x in touchP])
        ...
        # plot the vertices and reachable area flatten()是numpy.ndarray.flatten的一个函数，即返回一个折叠成一维的数组
        ...
        ax.plot3D(touchPx.flatten(), touchPz.flatten(), touchPy.flatten(), '.', color='#ff9557', label='Pinky')
...
try:
    name = sys.argv[1]
    with open("D:/VS_workspace/GraspSimulator/Assets/Models/HandR_v1/Data/Reachable_P.txt", "r") as infile:
        touchP = [line.rstrip().split(" ") for line in infile]
        print("touchP = ", touchP)
except:
    # with open("../Results/_p0_r0_0_0_1.txt", "r")  as infile:
    with open("D:/VS_workspace/GraspSimulator/Assets/Models/HandR_v1/Data/Reachable_P.txt", "r") as infile:
...
# Points are stored in the "x y z \n" format, convert into float
for i in range(len(touchP)):
    for j in range(len(touchP[i])):
        touchP[i][j] = float(touchP[i][j])

# Get the xy, xz, yz points
touchPxy = np.array([np.array([x[0]] + [x[1]]) for x in touchP])
touchPxz = np.array([np.array([x[0]] + [x[2]]) for x in touchP])
touchPyz = np.array([np.array([x[1]] + [x[2]]) for x in touchP])

# to remove points much bigger than surface area

for i in range(len(touchPxy)):
    if touchPxy[i][0] > xy_xmax:
        touchPxy[i][0] = xy_xmax

    if touchPxy[i][1] > xy_ymax:
        touchPxy[i][1] = xy_ymax

for i in range(len(touchPyz)):
    if touchPyz[i][0] > yz_xmax:
        touchPyz[i][0] = yz_xmax - 0.2

    if touchPyz[i][1] > yz_ymax:
        touchPyz[i][1] = yz_ymax - 0.2

for i in range(len(touchPxz)):
    if touchPxz[i][0] > xz_xmax:
        touchPxz[i][0] = xz_xmax - 0.2

    if touchPxz[i][1] > xz_ymax:
        touchPxz[i][1] = xz_ymax - 0.2
...
oxy, txy, ixy, mxy, rxy, pxy = computeReachArea(verticesxy, touchTxy, touchIxy, touchMxy, touchRxy, touchPxy, 131, 'X', 'Y')
plt.title('Front', fontsize=10)

# XZ
# oxz = computeReachArea(verticesxz, 132, 'X', 'Z')
oxz, txz, ixz, mxz, rxz, pxz = computeReachArea(verticesxz, touchTxz, touchIxz, touchMxz, touchRxz, touchPxz,132, 'X', 'Z')
plt.title('Top', fontsize=10)

# YZ
# oyz = computeReachArea(verticesyz, 133, 'Y', 'Z')
oyz, tyz, iyz, myz, ryz, pyz = computeReachArea(verticesyz, touchTyz, touchIyz, touchMyz, touchRyz, touchPyz,133, 'Y', 'Z')
...
c = ConvexHull3D(vertices, touchT, touchI, touchM, touchR, touchP)
...
textfile1 = Path("D:/VS_workspace/GraspSimulator/Assets/Models/HandR_v1/Results/computed_data1.txt")
...
# file1 = open("D:/VS_workspace/GraspSimulator/Assets/Models/HandR_v1/Results/computed_data1.txt", 'w')
# file1.write(oxy.split(":")[1].strip() + '\n')
# file1.write(oxz.split(":")[1].strip() + '\n')
# file1.write(oyz.split(":")[1].strip() + '\n')
# file1.write(txy.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(txy.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(txz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(txz.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(tyz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(tyz.split("(")[1].split("%")[0]).replace('.','') + '\n')
#
# file1.write(ixy.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(ixy.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(ixz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(ixz.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(iyz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(iyz.split("(")[1].split("%")[0]).replace('.','') + '\n')
#
# file1.write(mxy.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(mxy.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(mxz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(mxz.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(myz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(myz.split("(")[1].split("%")[0]).replace('.','') + '\n')
#
# file1.write(rxy.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(rxy.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(rxz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(rxz.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(ryz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(ryz.split("(")[1].split("%")[0]).replace('.','') + '\n')
#
# file1.write(pxy.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(pxy.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(pxz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(pxz.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.write(pyz.split(":")[1].strip().split("(")[0].strip() + '\n')
# file1.write('0.' + str(pyz.split("(")[1].split("%")[0]).replace('.','') + '\n')
# file1.close()
